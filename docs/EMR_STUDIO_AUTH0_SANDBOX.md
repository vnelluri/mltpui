...
| 5 | **ABAC via the session tag** | With alice's **federated session credentials** (NOT from a notebook cell — kernels run under the app's runtime role, not the user's session): `get-secret-value` on alice's secret, then bob's; ALSO test the per-user `iam:PassRole` condition (`…role/…-user-${aws:PrincipalTag/email}-runtime`) — the Tier-2 mitigation's core | Alice's secret read succeeds, bob's → `AccessDenied`; PassRole allows only alice's own runtime role — proves `PrincipalTag:email` arrived and the Tier-2 design enforces |
