...
- `backend/iac/`, `frontend/iac/` — Terraform modules (ECS Fargate). The EMR Studio module now lives in the companion `tmt-dataplane` repo (`modules/emr-studio`), which applies it; the backend only consumes its outputs.
