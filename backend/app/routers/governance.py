...
from app.dependencies import get_current_user, require_any_role, require_role
...
def submit_decision(
    ...
    request: Request,
    # Segregation of duties: the DECISION is MRM's alone. require_any_role
    # does not implicitly admit PlatformAdmin — the platform operator must
    # not be able to approve models on the reviewer's behalf.
    user: CurrentUser = Depends(require_any_role(["MRM"])),
) -> GovernanceReview:
